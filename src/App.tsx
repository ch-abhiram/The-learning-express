...
export default function App() {
  ...
  useEffect(() => {
    const scrollToTop = () => {
      ...
      const topElement = document.getElementById('top-anchor');
      if (topElement) {
  ...
  const handlePrevious = () => {
    if (currentArticleIndex > 0) {
      setSelectedArticleId(indianExpressEdition.articles[currentArticleIndex - 1].id);
    }
  };

  const handleNext = () => {
    if (currentArticleIndex < indianExpressEdition.articles.length - 1) {
      setSelectedArticleId(indianExpressEdition.articles[currentArticleIndex + 1].id);
    }
  };
  ...
  return (
    <div
      id="top-anchor"
      className={`min-h-screen bg-[#FBF9F5] text-[#24211D] flex flex-col justify-between selection:bg-[#E2D4B7] selection:text-[#1F1C18] ${
    ...
    >
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 py-8 w-full flex-1">
        ...
        <div className="space-y-12">
          {displayedArticles.map((article) => (
            <ArticleView
              key={article.id}
              article={article}
              userExplanation={explanations[article.id] || ''}
        ...
        {selectedArticleId !== 'all' && (
          <div className="flex items-center justify-between gap-4 border-t-2 border-[#24211D] pt-6 mb-8">
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-lg font-bold text-xs sm:text-sm transition-all cursor-pointer ${
                currentArticleIndex > 0
                  ? 'bg-[#24211D] text-white hover:bg-[#3E382E]'
            ...
            <button
              id="btn-view-all-stories"
              onClick={() => setSelectedArticleId('all')}
              className="text-xs sm:text-sm font-bold text-neutral-700 hover:text-black underline cursor-pointer"
            >
              View All Stories
            </button>
            ...
            <button
              id="btn-next-article"
              disabled={currentArticleIndex >= indianExpressEdition.articles.length - 1}
              onClick={handleNext}
              className={`flex items-center gap-2 px-4 py-2 rounded-lg font-bold text-xs sm:text-sm transition-all cursor-pointer ${
                currentArticleIndex < indianExpressEdition.articles.length - 1
                  ? 'bg-[#24211D] text-white hover:bg-[#3E382E]'
                  : 'bg-neutral-200 text-neutral-400 cursor-not-allowed'
              }`}
            >
      ...
      <footer className="border-t-4 border-[#24211D] bg-[#EFEAD8] py-8 text-center text-xs text-[#52493A] mt-10">
        <div className="max-w-4xl mx-auto px-4 space-y-3">
          ...
          <p className="text-[11px] font-mono text-[#6E614E]">
            "Understand the news. Connect the dots. Learn without fear."
          </p>
        </div>
      </footer>
    </div>
